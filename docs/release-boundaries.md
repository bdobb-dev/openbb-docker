...
- **v10.1.0** (cut 2026-09-14): `v10.0.0` + the kdb-line follow-ons, taken
  as ONE three-way merge of `93fcd88` (the last kdb commit before the Delta
  re-cut: tick-log durability, the kdb-ws demo chart, per-bar vwap, the
  `subscriptions` and `kdb_ticks` widgets) rather than 99 cherry-picks — the
  tag and that line had diverged since the Ep. 10 merge. Every conflict took
  93fcd88's side, since the retrofit layers the tag got by cherry-pick
