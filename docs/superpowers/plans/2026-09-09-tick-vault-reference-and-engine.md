...
**NAS access path (probed 2026-09-10):** The Cowork sandbox CANNOT reach the NAS — the NAS's tailnet IP (redacted; see `tailscale status` on the Mac) is unroutable directly, and the sandbox's egress proxy authenticates but returns Forbidden for that destination (same policy that blocks pypi/github); no host SSH keys are mounted, but that is moot given the network block. **The supported path is SSH from the Mac: `admin1@nas5c357c` (tailnet).** Consequences for this plan: reading legacy `ticks_sip/_progress/*.json` wall_minutes (Task 10's calibrate step), the Phase-0 live smoke, and the calibration week all run at the desktop — either directly on the NAS over SSH, or on the Mac against the NAS share — not from a Cowork sandbox. `vault calibrate --legacy-root` should accept both a mounted-share path and an `ssh://admin1@nas5c357c/...` style indirection is NOT required — keep it simple: run it where the paths are local (Mac with share mounted, or on the NAS itself via `docker exec`/ssh).
