...
- **OpenBB Platform** (AGPL-3.0-only) — the application every image serves.
  This repository is AGPL-3.0-only for that reason; its modifications to
  OpenBB (the CFTC startup guard in the Dockerfile, `api_app.py`) are in this
  tree, which is the source offer the licence requires.
...
def main(argv: list[str]) -> int:
    ...
    doc = [
        "# Third-party notices",
        "",
        "openbb-docker is AGPL-3.0-only (see `LICENSE`). The images it builds bundle the",
