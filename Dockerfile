...
# here by its `EPISODE=10 pnpm apps:sync`, which routes every example to the
# backend that owns its widgets (live-grid/apps.json holds the Ep. 10 one);
# baked in so this episode's image carries its own set.
